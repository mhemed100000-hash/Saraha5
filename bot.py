...
async def cmd_leaderboard(update,context):
    ...
    medals=["①","②","③"]
    lines=[f"{medals[i] if i<3 else f'#{i+1}'} *{d['name']}* — فوز {d.get('wins',0)} · قتل {d.get('kills',0)} · MVP {d.get('mvp',0)}" for i,(u,d) in enumerate(sp)]
    ...
    try:
        ...
        if g.votes:
            ...
            for vi,ti in g.votes.items():
                ...
                else: tt=g.get_player(ti); vr+=f"  ● {vt.name} → {tt.name if tt else '?'}\n"
            for p in alive:
