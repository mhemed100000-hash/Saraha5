        ...
        if context.args and len(context.args)>0 and context.args[0].startswith("join_"):
            try:
                ...
                if not g or g.phase!=Phase.LOBBY:
                    await update.message.reply_text("⚠️ اللعبة غير متاحة حالياً!\n\n🏰 ارجع للمجموعة وانضم من جديد.")
                    return
                ...
                if g.add_player(u.id,name):
                    ...
                    await update.message.reply_text("⚠️ اللعبة ممتلئة أو غير متاحة!")
            except Exception as e:
                logger.error(f"Deep link join error: {e}")
                await update.message.reply_text("⚠️ حصل خطأ! ارجع للمجموعة وحاول مرة ثانية.")
            return
    ...
    if d.startswith("join_"):
        ...
        if g.add_player(u.id,name):
            ...
            if not test:
                ...
                await q.answer("⚠️ اضغط الزر لفتح البوت أولاً!",show_alert=True)
                try: await context.bot.send_message(chat_id=cid,text=f"⚠️ *{name}*، افتح البوت أولاً ثم ارجع انضم!\n👇 اضغط الزر:",parse_mode="Markdown",reply_markup=kb)
                except: pass
                return
