...
class Game:
    ...
    MIN_PLAYERS = 4
    MAX_PLAYERS = 18
    NIGHT_DURATION = 90  # seconds (longer to give time for abilities)
    VOTE_DURATION = 90
...
async def join_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("join_"):
        ...
        if game.add_player(user.id, name):
            # Test if we can DM this user
            test_dm = await safe_send_dm(
                context.bot, user.id,
                f"✅ تم انضمامك للعبة Ghosts Palace!\n"
                f"⏳ انتظر بدء اللعبة لاستلام دورك."
            )
            if not test_dm:
                # Remove player since we can't DM them
                if user.id in game.players:
                    del game.players[user.id]
                bot_info = await context.bot.get_me()
                await query.answer(
                    f"⚠️ لم أستطع إرسال رسالة لك!\n"
                    f"افتح البوت أولاً @{bot_info.username} واضغط Start ثم انضم مرة ثانية.",
                    show_alert=True,
                )
                return
...
async def start_game_logic(bot: Bot, game: Game, chat_id: int, context: ContextTypes.DEFAULT_TYPE):
    ...
    if dm_failures:
        ...
        await safe_send(
            bot, chat_id,
            "⚠️ *تنبيه مهم!*\n\n"
            "لم أستطع إرسال الدور لهؤلاء اللاعبين:\n"
            + "\n".join(f"• {n}" for n in dm_failures)
            + f"\n\n📩 يجب أن يفتحوا البوت أولاً ويضغطوا Start:\n"
            f"👉 https://t.me/{bot_username}\n\n"
            "⏳ بعدها أعيدوا اللعبة من جديد بـ /start",
            parse_mode="Markdown",
        )
        # Cancel the game since not everyone got their roles
...
async def run_night(bot: Bot, game: Game):
    try:
        ...
        # Wait for night actions - split into 3 parts with reminders
        third = Game.NIGHT_DURATION // 3
        ...
        await safe_send(
            bot, game.chat_id,
            "⏳ الوقت يمر…\n"
            f"⚠️ متبقي {third * 2} ثانية!\n"
            "🎭 استخدموا قدراتكم الآن!"
        )
...
async def run_voting(bot: Bot, game: Game):
    try:
        ...
        # Announce voting start in group (no buttons)
        await safe_send(
            bot, game.chat_id,
            "⚖️ *بدأ التصويت!*\n\n"
            "📩 تم إرسال أزرار التصويت في الخاص لكل لاعب.\n"
            f"⏳ لديكم {Game.VOTE_DURATION} ثانية للتصويت!\n\n"
            "⚖️ كل صوت مهم… كل قرار قد يغير مصير اللعبة!\n"
            "🔒 التصويت سري… لكن النتيجة ستكشف الجميع!",
            parse_mode="Markdown",
        )

        # Send vote buttons to each alive player via DM
        for player in alive:
            # Build buttons excluding self
            buttons = []
            row = []
            targets = [p for p in alive if p.user_id != player.user_id]
            for i, t in enumerate(targets):
                row.append(InlineKeyboardButton(
                    f"{t.name}", callback_data=f"vote_{game.chat_id}_{t.user_id}"
                ))
                if len(row) == 2 or i == len(targets) - 1:
                    buttons.append(row)
                    row = []
            buttons.append([InlineKeyboardButton("⏭️ امتناع", callback_data=f"vote_{game.chat_id}_skip")])

            await safe_send_dm(
                bot, player.user_id,
                "⚖️ *وقت التصويت!*\n\n"
                "🎯 اختر الشخص الذي تعتقد أنه الأخطر:\n"
                f"⏳ لديك {Game.VOTE_DURATION} ثانية!",
                parse_mode="Markdown",
                reply_markup=InlineKeyboardMarkup(buttons),
            )

        # First reminder
        await asyncio.sleep(Game.VOTE_DURATION // 3)

        remaining1 = Game.VOTE_DURATION - (Game.VOTE_DURATION // 3)
        await safe_send(
            bot, game.chat_id,
            f"⚡ العد التنازلي مستمر!\n"
            f"⏱️ {remaining1} ثانية متبقية…\n"
            "📩 صوّتوا في الخاص الآن!"
        )

        # Second reminder
        await asyncio.sleep(Game.VOTE_DURATION // 3)

        remaining2 = Game.VOTE_DURATION // 3
        await safe_send(
            bot, game.chat_id,
            f"🔥 *آخر {remaining2} ثانية!*\n"
            "⏳ صوّتوا الآن قبل فوات الأوان!",
            parse_mode="Markdown",
        )

        # Remind players who haven't voted yet via DM
        for player in alive:
            if player.user_id not in game.votes:
                await safe_send_dm(
                    bot, player.user_id,
                    f"⚠️ *لم تصوّت بعد!*\n⏳ متبقي {remaining2} ثانية فقط!",
                    parse_mode="Markdown",
                )
        ...
        for p in alive:
            if p.role == RoleName.MADMAN and p.alive and p.user_id not in game.votes:
                ...
                if possible:
                    random_target = random.choice(possible)
                    game.votes[p.user_id] = random_target.user_id

        # Announce all votes publicly in group
        if game.votes:
            vote_reveal = "🗳️ *نتائج التصويت:*\n\n"
            for voter_id, target_id in game.votes.items():
                voter = game.get_player(voter_id)
                if target_id == -1:
                    vote_reveal += f"⚪ {voter.name} ← امتنع\n"
                else:
                    target = game.get_player(target_id)
                    vote_reveal += f"🗳️ {voter.name} صوّت ضد ➡️ {target.name if target else '?'}\n"

            # Show who didn't vote
            for player in alive:
                if player.user_id not in game.votes:
                    vote_reveal += f"❌ {player.name} ← لم يصوّت\n"

            await safe_send(bot, game.chat_id, vote_reveal, parse_mode="Markdown")
...
async def vote_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not voter:
        await query.answer("⚠️ أنت لست في هذه اللعبة!", show_alert=True)
        return

    if not voter.alive:
        await query.answer("💀 أنت ميت! لا يمكنك التصويت.", show_alert=True)
        return

    # ── One vote only! ──
    if voter_id in game.votes:
        await query.answer("⚠️ لقد صوّتَ بالفعل! لا يمكنك التصويت مرتين.", show_alert=True)
        return

    # Madman always votes randomly
    if voter.role == RoleName.MADMAN:
        possible = [p for p in game.alive_players if p.user_id != voter_id]
        if possible:
            random_target = random.choice(possible)
            game.votes[voter_id] = random_target.user_id
            await query.edit_message_text(
                f"🤡 تم تسجيل تصويتك!\n"
                f"(تصويتك عشوائي بسبب دورك كمجنون الغابة)"
            )
        return

    if target_value == "skip":
        game.votes[voter_id] = -1
        await query.edit_message_text("⏭️ تم تسجيل امتناعك عن التصويت.")
        return
    ...
    await query.edit_message_text(
        f"✅ تم تسجيل تصويتك ضد *{target.name if target else '?'}*\n"
        f"⏳ انتظر انتهاء وقت التصويت لرؤية النتائج في القروب.",
        parse_mode="Markdown",
    )
...
def main():
    ...
    # Register commands menu (so they show in Telegram hints)
    async def post_init(application):
        from telegram import BotCommand
        await application.bot.set_my_commands([
            BotCommand("start", "🏰 بدء لعبة جديدة"),
            BotCommand("join", "🎮 الانضمام للعبة"),
            BotCommand("startgame", "🚀 بدء اللعبة"),
            BotCommand("players", "👥 عرض اللاعبين"),
            BotCommand("endgame", "🛑 إنهاء اللعبة"),
        ])
    ...
    logger.info("🏰 Ghosts Palace Bot is starting...")
