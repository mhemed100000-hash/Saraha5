...
GIFS={
    "start":"https://media.giphy.com/media/l0HlBO7eyXzSZkJri/giphy.gif",
    "night":"https://media.giphy.com/media/3o7TKtnuHOHHUjR38Y/giphy.gif",
    "day":"https://media.giphy.com/media/l0MYt5jPR6QX5pnqM/giphy.gif",
    "vote":"https://media.giphy.com/media/26BRuo6sLetdllPAQ/giphy.gif",
    "death_criminal":"https://media.giphy.com/media/xT9IgG50Fb7Mi0prBC/giphy.gif",
    "death_ghost":"https://media.giphy.com/media/l0HlOvJ7yaacpuSas/giphy.gif",
    "death_poison":"https://media.giphy.com/media/3oEjHGr1Fhz0kyv8Ig/giphy.gif",
    "death_batman":"https://media.giphy.com/media/l3q2XhfQ8oCkm1Ts4/giphy.gif",
    "death_generic":"https://media.giphy.com/media/xT0xeJpnrWC3XWblEk/giphy.gif",
    "death_vote":"https://media.giphy.com/media/xT9IgG50Fb7Mi0prBC/giphy.gif",
    "win_village":"https://media.giphy.com/media/26u4cqiYI30juCOGY/giphy.gif",
    "win_evil":"https://media.giphy.com/media/l0HlBO7eyXzSZkJri/giphy.gif",
    "win_dracula":"https://media.giphy.com/media/l2JejfEHGGBRhifi8/giphy.gif",
    "truce":"https://media.giphy.com/media/26BRBKqUiq586bRVm/giphy.gif",
    "dog_transform":"https://media.giphy.com/media/3o7TKwmnDgQb5jemjK/giphy.gif",
    "blood_moon":"https://media.giphy.com/media/l0HlBO7eyXzSZkJri/giphy.gif",
    "storm":"https://media.giphy.com/media/3o7TKwmnDgQb5jemjK/giphy.gif"
}
...
def lobby_text(g):
    pl="\n".join(f"• {p.name}" for p in g.players.values()) or "• لا يوجد لاعبين بعد"
    c=len(g.players)
    st="🟢 يمكن بدء اللعبة!" if c>=Game.MIN_PLAYERS else f"⏳ يحتاج {Game.MIN_PLAYERS-c} لاعبين إضافيين"
    return (
        "🏰 *Ghosts Palace - 𝒉𝒂𝒗𝒂𝒏𝒂 𝑪𝒉𝒂𝒕🦩*\n"
        "━━━━━━━━━━━━━━━━━━━━\n\n"
        "🌑 *لعبة جديدة بدأت*\n"
        "👻 اجمعوا الشجاعة لما هو قادم\n\n"
        f"👥 *اللاعبون ({c}/{Game.MAX_PLAYERS}):*\n{pl}\n\n"
        f"{st}\n"
        "━━━━━━━━━━━━━━━━━━━━"
    )
...
async def run_vote(bot,g):
    try:
        ...
        await sgif(bot,g.chat_id,"vote","⚖️ *بدأ التصويت!*\n\n📩 أزرار التصويت وصلت إلى الخاص.\n⏳ {0} ثانية فقط.\n🔒 السرية مؤقتة… والنتيجة ستظهر للجميع.".format(Game.VOTE_DUR),parse_mode="Markdown")
        for p in alive:
            ...
            await sdm(bot,p.user_id,f"⚖️ *صوّت الآن!*\n🎯 اختر الأخطر:\n⏳ أمامك {Game.VOTE_DUR} ثانية.",parse_mode="Markdown",reply_markup=InlineKeyboardMarkup(btns))
        th=Game.VOTE_DUR//3
        await asyncio.sleep(th)
        ...
        if g.votes:
            ...
            for vi,ti in g.votes.items():
                ...
                if ti==-1:
                    ...
                    tt=g.get_player(ti)
                    vr += f"• {vt.name} → {tt.name if tt else '?'}\n"
            for p in alive:
                if p.user_id not in g.votes:
                    vr += f"• {p.name} → لم يصوّت\n"
            vr += "\n━━━━━━━━━━━━━━━━"
...
async def resolve_votes(bot,g):
    ...
    for u,c in sorted(vc.items(),key=lambda x:-x[1]):
        p=g.get_player(u)
        if p:
            bar="▓"*c+"░"*(max(len(g.alive_players)-c,0))
            slines.append(f"• {p.name}: {bar} ({c})")
    summary="\n".join(slines)
    if len(tops)>1:
        await ssend(bot,g.chat_id,f"⚖️ *نتائج التصويت:*\n\n{summary}\n\n⚖️ تعادل! لا يوجد إعدام هذه الجولة.\n🌙 استعدوا للجولة التالية…",parse_mode="Markdown")
        g._task=asyncio.create_task(run_night(bot,g)); return
...
async def announce_win(bot,g,winner,gn=""):
    g.phase=Phase.GAME_OVER
    if winner=="village": txt="🏆 انتهت الجولة!\n🎉 فاز الأبرياء!\n🛡️ تم كشف الأشرار.\n🌲 عاد الهدوء إلى القصر."; gk="win_village"; wt=Team.VILLAGER
    elif winner=="evil": txt="💀 انتهت الجولة!\n😈 فاز الأشرار!\n🩸 سيطر الظلام…\n🌑 لا أحد نجا من الليل."; gk="win_evil"; wt=Team.EVIL
    elif winner=="dracula": txt="🧛‍♂️ انتهت الجولة!\n🩸 دراكولا انتصر…\n⚰️ مرحبًا بالظلام الأبدي."; gk="win_dracula"; wt=Team.DRACULA
    elif winner=="grocer": txt=f"🧌 انتهت الجولة!\n{gn} فاز وحده!!\n💀 الكل خسر…"; gk="win_evil"; wt=Team.SOLO
