...
async def run_voting(bot: Bot, game: Game):
    try:
        ...
        msg = await safe_send(
            ...
            "👁️ التصويت علني — الجميع سيعرف من صوّت على من!",
            parse_mode="Markdown",
            reply_markup=InlineKeyboardMarkup(buttons),
        )
        if msg:
        ...
        for p in alive:
            if p.role == RoleName.MADMAN and p.alive and p.user_id not in game.votes:
                ...
                if possible:
                    ...
                    await safe_send(
                        bot, game.chat_id,
                        f"🗳️ {p.name} صوّت ضد ➡️ {random_target.name}"
                    )
...
async def vote_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if voter.role == RoleName.MADMAN:
        ...
        if possible:
            ...
            await query.answer(f"🤡 تم التصويت!", show_alert=True)
            # Show publicly (but don't reveal it's random)
            await safe_send(
                context.bot, chat_id,
                f"🗳️ *{voter.name}* صوّت ضد ➡️ *{random_target.name}*",
                parse_mode="Markdown",
            )
        return
    ...
    if target_value == "skip":
        ...
        await query.answer("⏭️ تم الامتناع!", show_alert=True)
        await safe_send(
            context.bot, chat_id,
            f"🗳️ *{voter.name}* امتنع عن التصويت ⚪",
            parse_mode="Markdown",
        )
        return
    ...
    if target_id == voter_id:
        await query.answer("⚠️ لا يمكنك التصويت لنفسك!", show_alert=True)
        return

    game.votes[voter_id] = target_id
    target = game.get_player(target_id)

    await query.answer(f"✅ تم التصويت ضد {target.name if target else '?'}!", show_alert=True)
    # ── Public announcement: who voted for whom ──
    await safe_send(
        context.bot, chat_id,
        f"🗳️ *{voter.name}* صوّت ضد ➡️ *{target.name if target else '?'}*",
        parse_mode="Markdown",
    )
