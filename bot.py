...
class Game:
    ...
    def __init__(self,cid):
        ...
        self._task=None; self._lobby_task=None; self._lobby_is_photo=False; self.log_entries:List[LogEntry]=[]; self.event=None
...
async def send_lobby_photo(bot,g):
    """Send cover image + lobby text + buttons as ONE message."""
    ...
    try:
        ...
        logger.warning(f"Lobby photo fail: {e}")
    # Fallback to text
...
async def update_lobby(bot,g):
    ...
    try:
        if g._lobby_is_photo:
            await bot.edit_message_caption(chat_id=g.chat_id,message_id=g.lobby_msg_id,caption=lobby_text(g),parse_mode="Markdown",reply_markup=lobby_kb(g))
        else:
            await bot.edit_message_text(chat_id=g.chat_id,message_id=g.lobby_msg_id,text=lobby_text(g),parse_mode="Markdown",reply_markup=lobby_kb(g))
    except: pass

async def lobby_refresh_loop(bot,g):
    """Re-sends lobby photo+text+buttons every 90 seconds so it stays visible."""
    try:
        while g.phase==Phase.LOBBY:
            ...
            # Re-send as single combined message
