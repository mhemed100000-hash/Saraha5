...
class Player:
    ...
    punishments: int = 0; completed: int = 0; played: bool = False
    muted_until: float = 0  # timestamp when mute ends (for auto-delete fallback)
    @property
    def tag(self):
...
class Game:
    def __init__(self, cid):
        ...
        self.challenge_msg_id: Optional[int] = None  # bot's message ID for the challenge
    ...
    def next_player(self):
        self.current_idx += 1
        self.current_type = None; self.current_challenge = ""; self.challenge_author = None; self.challenge_writer = None; self.challenge_msg_id = None
...
async def execute_punishment(bot, g, player):
    """Mute player. If can't mute (admin), mark them for auto-delete."""
    import time as _time
    cid = g.chat_id
    dur_min = random.randint(MUTE_MIN, MUTE_MAX)
    dur_sec = dur_min * 60
    player.muted_until = _time.time() + dur_sec
    player.punishments += 1

    try:
        from datetime import datetime, timedelta
        from telegram import ChatPermissions
        until = datetime.utcnow() + timedelta(seconds=dur_sec)
        await bot.restrict_chat_member(
            cid, player.user_id,
            permissions=ChatPermissions(can_send_messages=False),
            until_date=until
        )
        return f"🔇 تم كتم {player.tag} لمدة *{dur_min} دقيقة*!"
    except TelegramError as e:
        logger.warning(f"Mute fail (likely admin): {e}")
        # Fallback: auto-delete messages for duration
        return (
            f"🔇 *{player.tag}* معاقب لـ *{dur_min} دقيقة*!\n"
            f"⚠ ما قدرت أكتمه (أدمن)، بس رح أحذف كل رسالة يكتبها!"
        )

async def unmute_all_on_end(bot, g):
    """Unmute all players when game ends."""
    from telegram import ChatPermissions
    ...
    for p in g.players.values():
        if p.muted_until > 0:
            try:
                await bot.restrict_chat_member(g.chat_id, p.user_id, permissions=perms)
...
async def lobby_refresh_loop(bot, g):
    """تجديد اللوبي كل 90 ثانية — لا يتوقف عند الخطأ."""
    try:
        while g.phase == Phase.LOBBY:
            await asyncio.sleep(90)
            if g.phase != Phase.LOBBY: break
            # حذف القديمة — تجاهل أي خطأ
            try:
                if g.lobby_msg_id:
                    await bot.delete_message(chat_id=g.chat_id, message_id=g.lobby_msg_id)
            except Exception as e:
                logger.info(f"Old lobby already gone: {e}")
            # إرسال الجديدة — مع إعادة محاولة إذا فشلت
            for attempt in range(3):
                try:
                    m = await send_lobby_photo(bot, g)
                    if m: break
                except Exception as e:
                    logger.warning(f"Lobby resend attempt {attempt+1} failed: {e}")
                await asyncio.sleep(5)
    except asyncio.CancelledError:
        pass
    except Exception as e:
        logger.error(f"Lobby refresh loop crashed: {e}", exc_info=True)
...
async def start_round(bot, g):
    ...
    # Skip muted players - they can't play
    if p.muted_until > _time.time():
        await ssend(bot, g.chat_id, f"⏭ {p.tag} لسا مكتوم — رح نتخطاه ونرجع إلو بعدين.",
                    parse_mode="Markdown")
...
async def ask_for_challenge(bot, g):
    ...
    # Pick a random player (not the current one, not currently muted) to write the challenge
    now = _time.time()
    others = [
        pl for pl in g.players.values()
        if pl.user_id != p.user_id and pl.muted_until <= now
    ]
    if not others:
        await ssend(bot, g.chat_id, "ما في حدا متاح يكتب التحدي (الكل معاقب أو ما في لاعبين كفاية)!\nننتقل للاعب التالي…")
        await asyncio.sleep(2)
        if g.next_player(): await start_round(bot, g)
        else: await end_game(bot, g)
        return
    ...
    await ssend(bot, g.chat_id,
        ...
        f"✍ {tdesc.get(g.current_type, 'اكتب تحدي لـ')} *{p.name}*\n"
...
async def present_challenge(bot, g):
    ...
    m = await ssend(bot, g.chat_id,
        f"{te.get(g.current_type, '🎭')} *التحدي لـ {p.name}*\n\n"
        f"📝 {g.current_challenge}\n\n"
        f"✍ من: {aname}\n"
        f"⏳ عندك *{CHALLENGE_TIME} ثانية* لتنفّذ!\n\n"
        f"💡 {p.tag} — اعمل *Reply* على هالرسالة (كلام/صورة/أي شي) ليبدأ التصويت فوراً!",
        parse_mode="Markdown")
    if m: g.challenge_msg_id = m.message_id
    if g._timeout_task and not g._timeout_task.done(): g._timeout_task.cancel()
...
async def start_judging(bot, g):
    ...
    author = g.players.get(g.challenge_author)
    aname = author.tag if author else "صاحب التحدي"
    kb = InlineKeyboardMarkup([[
    ...
    await ssend(bot, g.chat_id,
        f"⚖ *وقت الحكم!*\n\n"
        f"هل {p.tag} نفّذ التحدي؟\n"
        f"👨‍⚖ الحكم لإلك يا {aname}!\n"
        f"⏳ عندك 30 ثانية.",
        parse_mode="Markdown", reply_markup=kb)
    if g._timeout_task and not g._timeout_task.done(): g._timeout_task.cancel()
...
async def end_game(bot, g):
    ...
    # Unmute everyone - punishments end with the game
    await unmute_all_on_end(bot, g)
    lines = []
    for p in g.players.values():
        if p.punishments == 0 and p.completed > 0: st = "💪 بطل"
        elif p.punishments > 0: st = f"😂 تعاقب {p.punishments} مرة"
        else: st = "😐 ما سوا شي"
        lines.append(f"  {p.tag}: {st}")
...
async def cmd_lobby(update, context):
    """إعادة إرسال اللوبي إذا اختفت."""
    if update.effective_chat.type == "private": return
    cid = update.effective_chat.id
    g = gm.get(cid)
    if not g or g.phase != Phase.LOBBY:
        await update.message.reply_text("ما في لعبة بمرحلة الانضمام حالياً.")
        return
    # احذف اللوبي القديمة إذا موجودة
    try:
        if g.lobby_msg_id:
            await context.bot.delete_message(chat_id=cid, message_id=g.lobby_msg_id)
    except: pass
    # أرسل جديدة
    await send_lobby_photo(context.bot, g)
    # أعد تشغيل حلقة التجديد إذا وقفت
    if not g._lobby_task or g._lobby_task.done():
        g._lobby_task = asyncio.create_task(lobby_refresh_loop(context.bot, g))
...
async def judging_cb(update, context):
    ...
    p = g.current_player
    # Only the player who wrote the challenge can judge
    if q.from_user.id != g.challenge_author:
        author = g.players.get(g.challenge_author)
        aname = author.name if author else "صاحب التحدي"
        await q.answer(f"بس {aname} بيقدر يحكم!", show_alert=True); return
    if g._timeout_task and not g._timeout_task.done(): g._timeout_task.cancel()
    voter = g.players.get(q.from_user.id)
    vname = voter.tag if voter else q.from_user.first_name
    if verdict == "yes":
        try: await q.edit_message_text(f"☑ {vname} قرر إنو {p.tag} نفّذ!", parse_mode="Markdown")
        except: pass
        await complete_challenge(context.bot, g)
    else:
        try: await q.edit_message_text(f"❌ {vname} قرر إنو {p.tag} ما نفّذ!", parse_mode="Markdown")
        except: pass
...
async def on_group_message(update, context):
    ...
    # 1) AUTO-DELETE for muted admins (works across all phases while game running)
    import time as _time
    p_muted = g.players.get(uid)
    if p_muted and p_muted.muted_until > _time.time():
        try:
            await msg.delete()
        except TelegramError as e:
    ...
    # 2) WAITING_CHALLENGE: the chosen writer submits a text challenge
    if g.phase == Phase.WAITING_CHALLENGE:
        p = g.current_player
        if not p: return
        if uid != g.challenge_writer: return
        if not msg.text: return  # must be text
        if g._timeout_task and not g._timeout_task.done(): g._timeout_task.cancel()
        g.current_challenge = msg.text
        g.challenge_author = uid
        await asyncio.sleep(1)
        await present_challenge(context.bot, g)
        return

    # 3) EXECUTING: current player replies to bot's challenge message → start voting NOW
    if g.phase == Phase.EXECUTING:
        p = g.current_player
        if not p: return
        if uid != p.user_id: return
        # Must be a reply to the bot's challenge message
        if not msg.reply_to_message: return
        if msg.reply_to_message.message_id != g.challenge_msg_id: return
        # Player responded — trigger voting immediately
        if g._timeout_task and not g._timeout_task.done(): g._timeout_task.cancel()
        await ssend(context.bot, g.chat_id, f"✅ {p.tag} رد على التحدي!", parse_mode="Markdown")
        await asyncio.sleep(1)
        await start_judging(context.bot, g)
        return
...
def main():
    ...
    async def post_init(application):
        ...
        await application.bot.set_my_commands([
            ...
            BotCommand("lobby", "إعادة إرسال اللوبي"),
    ...
    for cmd, fn in [("start", cmd_start), ("endgame", cmd_endgame), ("stats", cmd_stats),
                    ("leaderboard", cmd_leaderboard), ("skip", cmd_skip), ("lobby", cmd_lobby)]:
        app.add_handler(CommandHandler(cmd, fn))
    app.add_handler(CallbackQueryHandler(cb_router))
    app.add_handler(MessageHandler(~filters.COMMAND & filters.ChatType.GROUPS, on_group_message))
