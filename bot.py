...
async def cmd_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        f"👥 الحد الأدنى: {Game.MIN_PLAYERS} لاعبين\n"
        f"👥 الحد الأقصى: {Game.MAX_PLAYERS} لاعبًا\n\n"
        "عندما يكتمل العدد، استخدم /startgame لبدء اللعبة!",
        parse_mode="Markdown",
    )
...
async def cmd_join(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if game.add_player(user.id, name):
        ...
        await update.message.reply_text(
            f"✅ {name} انضم للعبة!\n"
            f"👥 عدد اللاعبين: {count}/{Game.MAX_PLAYERS}\n"
            f"{'🟢 يمكن بدء اللعبة!' if count >= Game.MIN_PLAYERS else f'⏳ يحتاج {Game.MIN_PLAYERS - count} لاعبين إضافيين'}"
        )
    else:
...
async def cmd_startgame(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for p in game.players.values():
        ...
        result = await safe_send_dm(context.bot, p.user_id, dm_text, parse_mode="Markdown")
        if not result:
    ...
    # Start night phase
    game._phase_task = asyncio.create_task(run_night(context.bot, game))
